...
def enrich_metadata(slugs: list[str]) -> list[dict]:
    ...
    try:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            futures = {pool.submit(_fetch_one, slug): slug for slug in pending}
    ...
    log.info(f"enrich_metadata: done. {len(already_done):,} succeeded, see {FAILED_PATH} for failures.")
